...
export default function Navbar({ children }) {
    ...
    return (
        <nav className="w-screen fixed top-0 z-10 ">
            <div className="max-w-screen flex flex-wrap items-center justify-between bg-[#FF0000] border-b-[10px] border-[#191F4F] mx-auto p-4 z-[1000]">
            ...
            {<div className={`absolute top-[70px] w-1/2  transition-all z-0 ${nav ? 'right-0' : 'right-[-550px]'} lg:w-auto lg:right-[35px] lg:top-[0px] lg:flex lg:h-[60px]`} >
                <ul className="h-[250px] lg:h-auto flex flex-col justify-around bg-[#FF0000] font-bold p-4 lg:p-0 mt-4 border border-gray-100 rounded-lg  lg:flex-row lg:space-x-8 lg:mt-0 lg:border-0 lg:flex-row lg:items-center">
                    <li>
                    ...
                    <li>
                        <a href="#Referencia" className={` lg:block py-2 pl-3 pr-4 text-white rounded  lg:hover:bg-transparent lg:p-0  ${active === 'Referencia' && 'text-black' }`} onClick={()=>pathnameHandler('Referencia')}>Referencia</a>
                    </li>
                    {/* <li>
